package com.monta.ocpp.emulator.ocpp.v201.connection
...
class ChargePointConnection(
    ...
    private suspend fun createConnection(
        ...
        client.webSocket(
            request = {
                ...
                header("Sec-WebSocket-Protocol", "ocpp2.0.1")
            },
        ) {
            websocketSession = this

            ocppClientV201.connect(
                ...
                for (frame in this.incoming) {
                    when (frame) {
                        is Frame.Text -> {
                            ...
                            if (newMessage != null) {
                                ocppClientV201.receiveMessage(chargePoint.identity, newMessage)
                            }
                            logLatency(message)
                        }
    ...
    suspend fun disconnect(
        ...
        try {
            ocppClientV201.asAvailability(chargePoint.sessionInfo).statusNotification(
                StatusNotificationRequest(
                    timestamp = ZonedDateTime.now(),
                    connectorStatus = StatusNotificationRequest.ConnectorStatus.Unavailable,
                    evseId = 1L,
                    connectorId = 1L,
                ),
            )
    ...
    private suspend fun handleReconnection(
        ...
        val shouldReconnect = try {
            ocppClientV201.onDisconnect(chargePoint.identity)
        } catch (exception: Exception) {
            true
        }
