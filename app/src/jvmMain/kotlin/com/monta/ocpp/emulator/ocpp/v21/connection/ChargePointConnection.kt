package com.monta.ocpp.emulator.ocpp.v21.connection
...
class ChargePointConnection(
                ...
                for (frame in this.incoming) {
                    when (frame) {
                        is Frame.Text -> {
                            ...
                            if (newMessage != null) {
                                ocppClientV21.receiveMessage(chargePoint.identity, newMessage)
                            }
                            logLatency(message)
                        }
    ...
    suspend fun disconnect(
        ...
        try {
            ocppClientV21.asAvailability(chargePoint.sessionInfo).statusNotification(
    ...
    private suspend fun handleReconnection(
        ...
        val shouldReconnect = try {
            ocppClientV21.onDisconnect(chargePoint.identity)
        } catch (exception: Exception) {
            true
        }
