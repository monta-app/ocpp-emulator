...
No navigation library: `NavigationViewModel` (a Koin singleton in `v16/.../common/view/`) holds a `Screen` sealed class in Compose `mutableStateOf`; `MainWindow` switches on it. View models are Koin singletons holding Compose state rather than per-screen instances. Shared reusable components live in `common/.../common/components/`, theming in `common/.../theme/` (`MontaTheme`).
